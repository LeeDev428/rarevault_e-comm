    ...
    async fetchMarketplaceItems(isLoadMore = false) {
      try {
        ...
        const params = new URLSearchParams({
          ...
          per_page: 20,
          sort_by: this.sortBy
          // No category filter - show all items that don't match other specific categories
        });
        ...
        // Filter out items that belong to specific categories (antiques, coins, collectibles)
        const filteredItems = (data.items || []).filter(item => {
          const category = (item.category || '').toLowerCase();
          return !['antiques', 'coins', 'collectibles'].includes(category);
        });
